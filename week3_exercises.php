...
// Läs in innehållet från filen om den finns
if (file_exists($filename)) {
    $data = file_get_contents($filename);
    $tasks = json_decode($data, true); // Konvertera JSON till associerad array
} else {
    $tasks = []; // Om filen inte finns, skapa en tom array
}

// Lägg till en ny uppgift (nyckel => värde)
$tasks['task3'] = 'Lägg till nya uppgifter';

// Spara tillbaka alla uppgifter till filen
file_put_contents($filename, json_encode($tasks, JSON_PRETTY_PRINT)); // Spara som JSON
?>
